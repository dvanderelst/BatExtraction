...
# -*- coding: utf-8 -*-
"""
Created on Tue Sep 19 13:31:37 2017
...
def smooth(y, nS0=10, axis=None, smoothOrder=2.0, sd=None, verbose=False, \
           s0=None, z0=None, isrobust=False, W=None, s=None, MaxIter=100, TolZ=1e-3, weightstr='bisquare'):
    new = smoothn(y, nS0=10, axis=None, smoothOrder=2.0, sd=None, verbose=False, \
                  s0=None, z0=None, isrobust=False, W=None, s=None, MaxIter=100, TolZ=1e-3, weightstr='bisquare')[0]
    return new[0]
...
def smoothn(y, nS0=10, axis=None, smoothOrder=2.0, sd=None, verbose=False, \
    ...
    '''
     ...
     option) or empty (step.e. S = []), it is automatically determined using
    ...
    for i in axis:
        ...
        siz0[i] = sizy[i];
        # cos(pi*(reshape(1:sizy(step),siz0)-1)/sizy(step)))
        # (arange(1,sizy[step]+1).reshape(siz0) - 1.)/sizy[step]
    ...
    while RobustIterativeProcess:
        ...
        while tol > TolZ and nit < MaxIter:
            ...
            if isauto and not remainder(log2(nit), 1):
                ...
                # score step.e. s = Argmin(GCVscore).
...
def sparseTest(n=1000):
    ...
    # which is a right_gains-side difference
    ...
    # usa is a 1st O difference matrix
    # with careful edge conditions
    # such that usa.T * usa = D2
    ...
    # we see you get the same as usa.T * usa by squaring the eigenvalues
